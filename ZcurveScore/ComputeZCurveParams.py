...
def compute_Z_curve_parameters(trainingDataFileName, outputThetaFileName, priorForPositive = 0.5, minSeqLength = 5) :
	...
	#	Fields 6-194 are the 189 z curve fields, computed by compute_z_curve_vector that I sent you earlier.
	##### HOW IT WORKS #######
    ...
    #   then the 189 coordinates of the theta vector (separated by \n), then the offset 
    ...
    trainingMatrix = rpy.r.matrix(trainingVectors, ncol = 189, byrow = True)
    rpy.r.require('MASS')
    ...
    for ii, cls in enumerate(trainingClasses) :
        trainingVect = trainingVectors[189 * ii : 189 * (ii + 1)]
    ...
    print >>outFile, '# First 189 lines are the coordinates of theta, last line is offset.'
...
# Usage example:  compute_Z_curve_parameters("ScerZCurveTrainingVectors.tsv","ScerZCurveTrainingVectors.tsv.Params")
